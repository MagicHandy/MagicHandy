...
$script:ParakeetModelURL = 'https://huggingface.co/mudler/parakeet-cpp-gguf/resolve/bf0af9f425fa01809cadec671b3cb672709d13e9/tdt-0.6b-v3-q4_k.gguf?download=true'
...
function Write-MagicHandySetupInstallResult {
    param(
        [Parameter(Mandatory = $true)][string]$DataDir,
        [Parameter(Mandatory = $true)][ValidateSet('complete', 'failed', 'cancelled')][string]$Status,
        [Parameter(Mandatory = $true)][string]$Message
    )

    $sanitized = [Regex]::Replace($Message, '[\x00-\x1f\x7f]', ' ')
    ...
    $record = [ordered]@{
        schema_version = 1
        job = [ordered]@{
            id = 'persisted-parakeet-source-install'
            kind = 'parakeet'
            module = 'parakeet'
            device = 'cpu'
            status = $Status
    ...
    $path = Join-Path $DataDir 'setup-last-install.json'
    $partial = "$path.partial-$PID"
    try {
        Write-MagicHandyUTF8 -Path $partial -Content ($record | ConvertTo-Json -Depth 5)
        Move-Item -LiteralPath $partial -Destination $path -Force
    } finally {
        if (Test-Path -LiteralPath $partial) {
            Remove-Item -LiteralPath $partial -Force -ErrorAction SilentlyContinue
...
function Invoke-MagicHandyParakeetInstall {
    param([Parameter(Mandatory = $true)][string]$DataDir)

    try {
        Install-MagicHandyParakeet -DataDir $DataDir
        try {
            Write-MagicHandySetupInstallResult -DataDir $DataDir -Status complete -Message 'Parakeet installed and verified.'
        } catch {
            Write-Warning "Parakeet installed, but its setup result could not be saved: $_"
        }
    } catch {
        $originalError = $_
        try {
            Write-MagicHandySetupInstallResult -DataDir $DataDir -Status failed -Message "Parakeet installation failed: $($originalError.Exception.Message)"
        } catch {
            Write-Warning "The Parakeet failure result could not be saved: $_"
        }
        throw $originalError
    }
}
...
function Invoke-MagicHandyProvision {
    ...
    if (-not $CoreOnly -and [bool]$State.install_parakeet) {
        ...
        Invoke-MagicHandyParakeetInstall -DataDir $State.data_dir
    }
...
Export-ModuleMember -Function @(
    ...
    'Invoke-MagicHandyParakeetInstall',
    'Sync-MagicHandyTTSModuleArtifacts',
