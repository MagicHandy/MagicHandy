...
reviewed alpha.14 through alpha.33 package-preserving releases.
...
3. **Alpha.8 through alpha.11 and alpha.13 through alpha.33 reviewed unsigned
   ...
   autonomous unsafe-state retry loop. None changes installer behavior or
   payload composition.
...
- `Test-WindowsRelease.ps1 -ArtifactPolicy ReviewedUnsignedPublic` requires an
  alpha.8 through alpha.11 or alpha.13 through alpha.33 version, the recorded
...
- Alpha.9 through alpha.11 and alpha.13 through alpha.33 reviewed setup
