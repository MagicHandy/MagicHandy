...
review. This supersedes ADR 0013 where that ADR defines public unsigned setup
publication.
...
3. **Alpha.8 reviewed unsigned setup is an explicit exception.** The tag workflow
   may publish the alpha.8 unsigned setup only with the
   `ReviewedUnsignedPublic` verification policy and the completed Microsoft
   case ID above. The verifier rejects every other version, so a later unsigned
...
- Microsoft's determination covers the submitted alpha.6 hash, not every future
  package; and
...
- `Test-WindowsRelease.ps1 -ArtifactPolicy ReviewedUnsignedPublic` requires the
  alpha.8 version and recorded Microsoft case ID, the setup/portable/checksum
  set, x64 PE headers, unsigned status, exact hashes, and supports the complete
  installer lifecycle.
