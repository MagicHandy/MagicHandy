...
| **AI display name, profile picture, splash personalization** (`ai_name`, `profile_picture_b64`, splash) | **Shipped 2026-07-29** as [persona-page.md](persona-page.md) slices 1-6. The display name and portrait are fields of a named preset over the existing personalization axes rather than loose cosmetic settings; the name enters the prompt as quoted, bounded data like every other profile field. Portraits are resized in the browser and stored in a purgeable directory beside video thumbnails. Splash personalization was not built. |
