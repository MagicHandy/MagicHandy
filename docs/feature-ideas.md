...
| **Autospeak** (`autospeak_enabled/min_seconds/max_seconds/motion_autonomy`) — the model speaks unprompted at a bounded random cadence, with a separate motion-autonomy level | **Worth scoping — this is Autopilot's voice half.** The plan already notes the autospeak contract shapes feed the Autopilot slice. Keep the STGPT-RV shape: bounded user-set cadence window, autonomy level as visible state, hard-off switch. |
...
| **Program section clipping + bookmarks** (Settings tour: "Program section clipping"; funscript timeline crop with device preview, history 2026-04-23) | **Partial → clip-on-import shipped (2026-07-19).** The library's Import tab now renders a funscript timeline with a dual-thumb trim and a pattern/program choice; the trimmed selection goes through the normal validated import. Play-this-selection device preview and named section bookmarks remain unbuilt. |
