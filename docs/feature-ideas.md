...
| **Autospeak** (`autospeak_enabled/min_seconds/max_seconds/motion_autonomy`) — the model speaks unprompted at a bounded random cadence, with a separate motion-autonomy level | **Implementation specified.** [autopilot-cadence.md](autopilot-cadence.md) keeps the useful bounded/off/authority shape while separating motion and speech clocks, using code-bounded timing categories, and scheduling from audible playback rather than model completion. |
...
| **AI display name, profile picture, splash personalization** (`ai_name`, `profile_picture_b64`, splash) | **Shipped 2026-07-29** as [persona-page.md](persona-page.md) slices 1-7. The display name and portrait are fields of a named preset over the existing personalization axes rather than loose cosmetic settings; bounded per-persona lore enters only interactive prompts under an explicit off/relevant/full policy and is inspectable in Diagnostics. Portraits are resized in the browser and stored in a purgeable directory beside video thumbnails. Splash personalization was not built. |
...
| **TTS style tuning knobs** (`local_tts_exaggeration/cfg_weight/temperature/top_p/min_p/repetition_penalty`) | **Deliberate non-goal for now.** This is the "voice tab grew 12 knobs before defaults were proven" anti-lesson the plan already cites. Faster Qwen now has a bounded set of reviewed instruction presets plus one custom tone prompt because its pinned Base API supports that semantic control; this is not a general low-level sampling panel. Add more provider-specific controls only after listening evidence (R17 discipline). |
