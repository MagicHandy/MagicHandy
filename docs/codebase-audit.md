...
| Media catalog and plain playback | Reviewed in Phase 18 M0 | Saved absolute roots feed only an explicit depth/file-bounded scanner; opaque catalog IDs and component-level symlink checks jail Range streaming. Videos and the reusable import-preview player do not import motion or transport. Real multi-folder and 2 GB stable-RSS acceptance remain open. |
