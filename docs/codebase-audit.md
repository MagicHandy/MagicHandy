...
Baseline: `origin/main` at `3a7ed0fb` (2026-07-20).

| Subsystem | Status | Current evidence |
| --- | --- | --- |
| Configuration and persistence boundaries | Reviewed in dedicated pass; account follow-up through schema v19 | One process-owned SQLite pool serves every logical domain; writes share one transaction lock. Schema v18 adds user accounts and opaque-session digests; v19 adds profile cache stamps, directional links, and per-session control context without a second datastore. Invalid settings are preserved, physical corruption is quarantined, logical damage fails clearly, and schema/version/permission/lifecycle behavior has focused coverage. |
...
| HTTP and process lifecycle | Reviewed in first pass; authenticated-LAN/account-GUI follow-up | Oversized JSON is rejected, response encoding cannot panic after committing headers, browser requests require an exact allowed same origin, and non-loopback startup requires exact-address TLS plus an enabled account. Opaque sessions, JSON login, role checks, profile visibility, linked-context validation, and login throttles wrap but never replace the existing controller lease; Emergency Stop remains reachable after auth expiry; shutdown still quiesces device work before closing stores. |
...
| Media catalog and synchronized playback | Reviewed through Phase 18 M2 | Saved roots feed only the explicit bounded scanner; opaque ids and rooted file handles jail Range streaming and exact-name funscript reads. The dedicated Videos workspace loads one bounded script for its canvas and the backend's finite linear media target. Browser-clock play/seek/pause/rate/end/close events are controller-gated; discontinuities and heartbeat loss Stop the shared engine, explicit arms carry the Emergency Stop generation, and heartbeats cannot start motion. Read-only and failed-script playback remain honest. Multi-root/2 GiB stream evidence covers M0; capped real-device alignment remains M3. |
| Frontend state, accessibility, and UI performance | Reviewed in dedicated pass; backend-owned chat workspace follow-up | Route lifetime preserves settings drafts; failed reads remain distinct from valid empty state; quick writes flush across unmount; chat tail reads retry; and persistence/mode mutations serialize before React rerenders. The Import timeline uses one measured coordinate system for waveform, selection, and fixed 44 px trim targets; zoom cannot mutate content, wheel zoom is cursor-anchored, and a proportional pointer/keyboard scrollbar directly moves the viewport. Long loops preserve duration, impossible essential-knot counts fail before upload, and compact previews retain saved reversals. Chat fills the routed workspace with stable keyboard-operable session tabs and its existing Autopilot/control sidebar; guarded Save/discard transitions preserve the single backend-active session, and assistant avatars expose persisted non-secret model provenance. Manual motion remains in Diagnostics and keys active state from backend target provenance. Full-suite count and final bundle measurement are recorded in the current scorecard entry. |
...
- Manual Motion treated every running engine as an active test, so starting
  ...
  shared engine. The manual-target endpoint additionally requires a live
  `manual_ui` owner, preventing a stale control from relabeling Autopilot or an
  idle engine. This is runtime provenance and lifecycle state, not persisted
