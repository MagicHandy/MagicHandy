...
| Media catalog and plain playback | Reviewed in Phase 18 M0 and the 2026-07-20 route/handling follow-up | Saved absolute roots feed only an explicit depth/file-bounded scanner; opaque catalog IDs, rooted file handles, file-identity checks, deterministic video MIME types, and response safety headers jail Range streaming. Videos is an independent routed workspace with resilient scan polling, stale-response guards, visible partial failures, and retryable native playback; the reusable import-preview player still does not import motion or transport. Multi-root scanning and a complete 2 GiB stable-RSS stream passed manual acceptance on 2026-07-19. |
