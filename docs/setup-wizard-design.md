...
This document describes the implemented six-step experience and the remaining
...
3. Model runtime
   - managed llama.cpp source build: auto / CPU / CUDA
...
The current managed option is a source build. Its card and install panel state
that it installs Git, CMake, Visual Studio C++ Build Tools, and possibly the
CUDA Toolkit, with a several-GiB temporary footprint. The benefit is a pinned,
app-owned runner whose startup, model loading, diagnostics, and shutdown are
under MagicHandy control.

The Ollama option is explicitly for an existing installation. It avoids the
managed compiler/runtime footprint and uses the user's existing daemon and
model library. External llama.cpp is similarly user-owned. The runtime choice
is saved when leaving the runtime step, so skipping model selection does not
silently discard it.
...
Parakeet is a separate install action with runner/model license and download
size shown. Reference WAV and exact transcript selection, provider tuning,
enabling voice, and starting workers remain in Settings > Voice, where they can
be tested with the rest of the voice controls.

## Installation Jobs

The backend owns one queue shared by managed llama.cpp, Parakeet, and local
TTS. A queued or running job blocks another install, exposes bounded output,
and has one Cancel action. The helper process tree is owned by the app and is
terminated on cancellation or server shutdown. Safe partial downloads remain
for retry.
...
Below 780 px the progress rail becomes a six-position top stepper. Below 560 px
...
- Every install is explicit, controller-gated, visible, cancellable, and
  leaves voice disabled.
