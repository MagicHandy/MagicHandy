...
one bottom accent rather than mixing a three-sided outline with the strip
baseline. Tabs use a generous preferred width, then compact evenly to a readable
minimum before horizontal overflow; the native scrollbar stays hidden while
wheel, touch, keyboard focus, and active-session changes keep overflow usable.
New Chat follows the rightmost tab in a dedicated trailing hit area; the icon is
borderless at rest, and only the tab list scrolls when it overflows, so the
