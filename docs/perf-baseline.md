...
| MagicHandy Go core idle, Phase 14C completed connection composition | Phase 14C working tree | plain and `-ldflags "-s -w"` `CGO_ENABLED=0` builds under `%TEMP%\MagicHandy-topbar-manager`; fresh stripped binary on `127.0.0.1:49770` with isolated data; `/healthz`, two-second warmup, then three one-second samples | No browser window for RSS; rendered UI checked separately | Yes; no device, model, or voice worker included | 53.63 MiB (56,238,080 bytes) across 3 equal samples | Not measured separately | Plain binary 19,636,736 bytes; stripped binary 13,746,176 bytes. JS 277,597 / 81,475 gzip; CSS 37,555 / 8,033 gzip; HTML 454 / 282 gzip; isolated PNG 444,236 / 437,397 gzip; total embedded browser payload 759,842 raw / 527,187 gzip. |
...
- The embedded production build was rendered at 1280×800 and 390×844. The
  trigger sits at the far right of the 48px top bar and the 360×658px panel
  opens at y=56 with no document, panel, or internal overflow; all four
  immediate limits remain visible.
- At 390×844 the complete panel ends at y=714 while the always-mounted Stop bar
  begins at y=748, preserving a 34px safety gap and clear bottom navigation.
- The artwork uses one 444,236-byte transparent hand isolation generated from
  ...
  three signal paths in the lower half, and the rounded Handy body/indicator;
  disconnected/error replaces the signal group with a two-stroke red X.
