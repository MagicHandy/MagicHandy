...
| MagicHandy chat-motion and quarantine-source audit | Current model-owned chat-motion working tree | plain and `-ldflags "-s -w"` `CGO_ENABLED=0` builds under `.scratch`; full Go/frontend/static checks plus isolated live prompt evaluation against the already-loaded managed Gemma model | No review process included in this size row; the live evaluator creates no engine or transport | Yes; measured binaries exclude the external model runner | RSS not re-measured; production behavior changes are prompt/authorization policy and offline review tooling | Not measured separately | Plain binary 22,950,400 bytes; stripped binary 16,414,720 bytes, +4,608 each from the immediately preceding context/quarantine tree and still below 30 MB. The canonical browser build is 1,535,379 raw / 756,133 gzip bytes (-15 / -11) after restoring the visible `experimental` status. The quarantined 171-file manifest shrank from 1,826 to 692 JSON lines while becoming filename/name complete; dead review assets remain outside the active catalog. |
