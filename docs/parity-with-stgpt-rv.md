...
| **LLM-driven autonomous mode (Autopilot)** | **Initial implementation (PR #101)** | Chat-native control; bounded recent-conversation context; enabled pattern-shape selection plus independent `speed_percent` pacing over the shared segment loop; visible deterministic fallback; chat-log/browser-TTS delivery. Live-model, long-session, cadence, and richer-arrangement acceptance remain open; see [llm-control-surface.md](llm-control-surface.md) ideas E/F |
