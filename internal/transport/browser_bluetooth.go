...
func (t *BrowserBluetoothTransport) Stop(ctx context.Context, command StopCommand) (CommandResult, error) {
	...
	return t.dispatch(ctx, recorded, "hsp/stop", nil)
}
...
func (t *BrowserBluetoothTransport) AddHSP(ctx context.Context, command HSPAddCommand) (CommandResult, error) {
	streamID, err := cleanBluetoothStreamID(command.StreamID)
	if err != nil {
	...
	return t.dispatch(ctx, recorded, "hsp/add", body)
}
...
func (t *BrowserBluetoothTransport) PlayHSP(ctx context.Context, command HSPPlayCommand) (CommandResult, error) {
	...
	return t.dispatch(ctx, recorded, "hsp/play", body)
}
...
func cleanBluetoothStreamID(streamID string) (int, error) {
	...
	if err != nil || id < 0 {
		return 0, errors.New("Bluetooth HSP stream ID must be a non-negative integer")
	}
	return id, nil
}
