...
func TestMotionUnavailableWithoutSelectedTransportPrerequisites(t *testing.T) {
	...
	if state.Available {
		t.Fatalf("motion state should be unavailable without cloud connection key: %s", stateRecorder.Body.String())
	}
	if !strings.Contains(stateRecorder.Body.String(), "Handy connection key is required") {
		t.Fatalf("state body = %s, want cloud connection key error", stateRecorder.Body.String())
	}
