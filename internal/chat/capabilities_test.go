...
func TestComposeSystemUsesAuthoritativeMotionContext(t *testing.T) {
	...
	for _, want := range []string{
		...
		`When embodied partner-action wording is a direct request`,
		`acknowledge the motion decision rather than declining the request`,
		`The current state is running: choose target or none; start is invalid`,
		`Make the reply match motion.action`,
