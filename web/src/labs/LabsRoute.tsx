...
import {LabHelp} from "./LabHelp";
import "../styles/motion-lab.css";

export function LabsRoute() {
  const requested=useHashRoute().split("/")[2];
  const section=requested==="motion"||requested==="observations"||requested==="tests"||requested==="help"?requested:"chat";
  ...
  return <div className="labs-route">
    ...
    <nav className="labs-tabs" aria-label={t("Lab workspaces")}>
      {([["chat",t("LLM Lab")],["motion",t("Motion Lab")],["tests",t("Guided tests")],["observations",t("Observations")],["help",t("Help")]] as const).map(([id,label])=><a key={id} href={`#/labs/${id}`} aria-current={section===id?"page":undefined}>{label}</a>)}
    </nav>
