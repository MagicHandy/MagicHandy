    ...
    <div className="modal-scrim" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      ...
      >
        <header className="reference-dialog-header">
          <div>
            <p className="eyebrow">{t("NeuTTS Air")}</p>
            <h2 id="neutts-reference-title">{t("Create reference voice")}</h2>
          </div>
          <button type="button" className="reference-dialog-close" aria-label={t("Close reference voice window")} title={t("Close")} onClick={onClose}>{"\u00d7"}</button>
        </header>

        <div className="reference-dialog-body">
          <p id="neutts-reference-description" className="form-status">{t("Generate NeuCodec reference codes locally from a WAV and its exact transcript. Python is not used.")}</p>
          <HostPathField
            label={t("Source voice (.wav)")}
            kind="wav"
            value={wavPath}
            disabled={generating}
            placeholder={t("Choose a clean 3-15 second voice sample")}
            onChange={(value) => { invalidateAudio(); setWAVPath(value); }}
          />
          <label className="field">
            <span className="label">{t("Exact source transcript")}</span>
            <textarea
              rows={4}
              value={transcript}
              disabled={generating}
              onChange={(event) => { setTranscript(event.target.value); setError(""); }}
              placeholder={t("Type exactly what the speaker says")}
            />
          </label>
          <div className="reference-guide">
            <strong>{t("Reference quality")}</strong>
            <ul>
              <li>{t("Use one speaker with little background noise and few long pauses.")}</li>
              <li>{t("Preserve contractions and numbers exactly as spoken.")}</li>
              <li>{t("Exclude speaker labels, timestamps, and stage directions.")}</li>
            </ul>
          </div>
          ...
          {generated && previewSource && (
            <div className="reference-transcription">
              <audio key={previewSource} controls preload="metadata" src={previewSource} aria-label={t("NeuTTS reference audio preview")} />
              <p className="form-status">{t("Listen once before applying. Correcting the transcript does not require regenerating the audio codes.")}</p>
            </div>
          )}
        </div>
        ...
        <footer className="reference-dialog-actions">
          ...
          <button type="button" className="btn btn-primary" disabled={!generated || !previewURL || !transcript.trim()} onClick={apply}>{t("Use reference")}</button>
        </footer>
      </section>
    </div>
  );
